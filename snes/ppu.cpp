...
static int ppu_getPixel(Ppu *ppu, int x, int y, bool sub, int *r, int *g, int *b, int *pixel);
...
int Ppu::GetCurrentRenderScale(uint32_t render_flags) {
...
static void PpuDrawMode7Upsampled(Ppu *ppu, uint y) {
	...
	if (*(uint32 *) &ppu->mode7PerspectiveLow == 0) {
		...
		for (int i = 0; i < 4; i++)
			m0v[i] = 4096.0f / FloatInterpolate((int) y + kInterpolateOffsets[i], 0, 223, ppu->mode7PerspectiveLow,
												ppu->mode7PerspectiveHigh);
	}
	...
	if (ppu->extraLeftRight - ppu->extraRightCur != 0) {
		...
		for (int i = 0; i < 4; i++)
			memset(render_buffer_ptr + pitch * i +
				   (256 + ppu->extraLeftRight * 2 - (ppu->extraLeftRight - ppu->extraRightCur)) * 4 * sizeof(uint32), 0,
				   n);
	}
#undef DRAW_PIXEL
}
...
//  0: backdrop
	...
	if (this->mode == 1) {
		if (this->lineHasSprites)
			PpuDrawSprites(this, y, sub, true);

		if (IS_MOSAIC_ENABLED(this, 0))
			PpuDrawBackground_4bpp_mosaic(this, y, sub, 0, 0xc000, 0x8000);
		else
			PpuDrawBackground_4bpp(this, y, sub, 0, 0xc000, 0x8000);

		if (IS_MOSAIC_ENABLED(this, 1))
			PpuDrawBackground_4bpp_mosaic(this, y, sub, 1, 0xb100, 0x7100);
		else
			PpuDrawBackground_4bpp(this, y, sub, 1, 0xb100, 0x7100);

		if (IS_MOSAIC_ENABLED(this, 2))
			PpuDrawBackground_2bpp_mosaic(this, y, sub, 2, 0xf200, 0x1200);
		else
			PpuDrawBackground_2bpp(this, y, sub, 2, 0xf200, 0x1200);
	} else {
		// mode 7
		PpuDrawBackground_mode7(this, y, sub, 0xc000);
		if (this->lineHasSprites)
			PpuDrawSprites(this, y, sub, false);
	}
}
...
static NOINLINE void PpuDrawWholeLine(Ppu *ppu, uint y) {
	if (ppu->forcedBlank) {
		...
		size_t n = sizeof(uint8) * (256 + ppu->extraLeftRight * 2);
		memset(dst, 0, n);
		return;
	}
	...
	uint8 *dst = (uint8 *) &ppu->renderBuffer[(y - 1) * ppu->renderPitch], *dst_org = dst;
	...
	do {
		...
		if (math_enabled_cur == 0 || fixed_color == 0 && !ppu->halfColor && !rendered_subscreen) {
			...
			uint32 i = left;
			do {
				dst[0] = ppu->bgBuffers[0].data[i] & 0xff;
				/*
			...
			do {
				...
				if (math_enabled_cur & (1 << main_layer)) {
					if (math_enabled_cur & 0x100) {
						if ((ppu->bgBuffers[1].data[i] & 0xff) != 0)
							dst[0] = ppu->bgBuffers[1].data[i] & 0xff;
						else
							dst[0] = ppu->bgBuffers[0].data[i] & 0xff;
					}
				} else {
...
static void ppu_handlePixel(Ppu *ppu, int x, int y) {
	...
	int pixel = 0;
	if (!ppu->forcedBlank) {
		int mainLayer = ppu_getPixel(ppu, x, y, false, &r, &g, &b, &pixel);
		...
		if ((mathEnabled && ppu->addSubscreen) || ppu->mode == 5 || ppu->mode == 6) {
			secondLayer = ppu_getPixel(ppu, x, y, true, &r2, &g2, &b2, &pixel);
		}
	...
	auto *pixelBuffer = (uint8 *) &ppu->renderBuffer[row * ppu->renderPitch + (x + ppu->extraLeftRight)/* *4 */];
...
//   0-3; layers 1-4; 4: sprites; 5: nonexistent
	...
	uint16_t color = ppu->cgram[pixelForColor & 0xff];
	*r = color & 0x1f;
	*g = (color >> 5) & 0x1f;
	*b = (color >> 10) & 0x1f;
